...
def make_sim_env(task_name, render_height=480, render_width=640):
    """
    ...
    if "sim_transfer_cube" in task_name:
        ...
        task = TransferCubeTask(render_height, render_width, random=False)
        ...
        physics = mujoco.Physics.from_xml_path(xml_path)
        task = InsertionTask(render_height, render_width, random=False)
        env = control.Environment(
            physics,
            task,
            time_limit=20,
...
class BimanualViperXTask(base.Task):
    def __init__(self, render_height: int, render_width: int, random=None):
...
class TransferCubeTask(BimanualViperXTask):
    def __init__(self, render_height: int, render_width: int, random=None):
        super().__init__(render_height, render_width, random=random)
        self.max_reward = 4
