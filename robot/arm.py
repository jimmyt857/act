...
import interbotix_xs_msgs.msg as interbotix_msgs
...
class InterbotixArmXSInterface(object):
    def __init__(self, core, robot_model, group_name, moving_time=2.0, accel_time=0.3):
        ...
        print(
            ...
            " Time-Based-Profile" % (group_name, moving_time, accel_time)
        )
    ...
    def publish_positions(
        self, positions, moving_time=None, accel_time=None, blocking=True
    ):
        self.set_trajectory_time(moving_time, accel_time)
        self.joint_commands = list(positions)
        joint_commands = interbotix_msgs.JointGroupCommand(
            self.group_name, self.joint_commands
        )
    ...
    def check_joint_limits(self, positions):
        ...
        for x in range(self.group_info.num_joints):
            if not (
                ...
                rospy.logwarn("Would exceed position limits on joint %s." % x)
                rospy.logwarn(
            ...
            if speed_list[x] > self.group_info.joint_velocity_limits[x]:
                rospy.logwarn("Would exceed velocity limits on joint %s." % x)
                rospy.logwarn(
    ...
    def set_joint_positions(
        self, joint_positions, moving_time=None, accel_time=None, blocking=True
    ):
    ...
    def set_single_joint_position(
        self, joint_name, position, moving_time=None, accel_time=None, blocking=True
    ):
        if not self.check_single_joint_limit(joint_name, position):
            return False
        self.set_trajectory_time(moving_time, accel_time)
        self.joint_commands[self.core.js_index_map[joint_name]] = position
        single_command = interbotix_msgs.JointSingleCommand(joint_name, position)
    ...
    def set_ee_pose_matrix(
        ...
        rospy.logwarn(
            "No valid pose could be found. Returned theta_list variable may be nonsense."
        )
    ...
    def set_ee_cartesian_trajectory(
        ...
        if success:
            ...
            self.core.pub_traj.publish(
                interbotix_msgs.JointTrajectoryCommand(
                    "group", self.group_name, joint_traj
                )
            )
    ...
    ### @return <bool> - True if a valid solution was found; False otherwise
    ### @details - Do not set 'yaw' if using an arm with fewer than 6dof
    def set_relative_ee_position_wrt_to_base_frame(
        self,
        *,
        dx=0,
        dy=0,
